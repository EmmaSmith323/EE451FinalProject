...
static void mutate(string* a,int index);
...
int main(int argc, char** argv) 
{
    ...
    //("n=%d, chunk_size = %d, fav= %s \n", n, chunk_size, fav);
    cout << "chunk_size " << chunk_size << "fav = " << fav <<"\n";
    ...
    while(!fav_capacity) 
    {
        ...
        for(int k =0; k<m_size*m_size/2 ; k++)
        {
            ...
            cout << "Mating Pool size " << matingPool.size() << "\n";
            ...
            cout << "FirstIndex " << firstIndex <<"Second index " << secondIndex <<"\n";
            cout << "Mating Pool size " << matingPool.size() << "\n";
...
void mutate(string* a, int index)
{
    string localString(a[index]);
    if (localString == "1")
        a[index] = "0";
    else
        a[index] = "1";
